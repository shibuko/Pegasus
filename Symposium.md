...
## 試験車両で実際の運転データを記録する準備
どうすれば運転データの記録を効率的におこなうことができるか？  

* 車両の量産品センサのデータを実際の交通状況下のシナリオ記録に使う。
* D-GPSシステムを絶対座標の取得に使う。
* 追加で前後のビデオを記録する。
* データの自動ダウンロード、自動変換、自動インポートでデータベースに取り込む（準備段階で）。
* 必要に応じてセンサを追加する（これについては議論中）。

# 13. Test Case generation, Test Space Coverage and Test Effort Reduction
## 環境シミュレーションとデータベースを繋ぎ合わせて、テスト空間に対する代表性のカバレッジを持つ具体シナリオの生成、実行、および評価の自動化を試みる。  
...
* 安全性の証明に向けたアクション
  * 危険シナリオの特定
  * テスト空間全体、統計分析、およびテストコンセプトに対する代表性のカバレッジ

### テスト自動化
* PEGASUSデータベースではパラメタ設定可能な論理シナリオとこれに対応するパラメタ空間を用意している。さらに、結果評価に必要な重要な指標（metrics）も含まれる（たとえば、TTC）
* QTronic TestWeaver、TraceTronic ECU-TEST、またはIfR Stochastic Moduleを使って、パラメタ空間からパラメタセットを引っ張ってきて、具体シナリオをOpenSCENARIO形式で生成する。前回までのシミュレーション結果を利用してパラメタを選択することができる（反応プロセス）。
* IPG CarMakerまたはdSPACE ModelDeskを使って具体シナリオを実行する。評価に向けて、シミュレーション結果をTestWeaverまたはECU-TESTにフィードバックする。

### パラメタ変動
* 論理シナリオのパラメタ空間を完全に網羅するようにテストするこはできない。つまり、テストを目的として、運転機能に対する所定の適用領域に関して、代表的サンプルを導出する必要がある。
* プロジェクトの前半では、確率論的モジュールを使って、インタフェース定義とフォーマット変換にフォーカスする。同時に、確率論的モジュールには次の機能がある：
  * 論理シナリオのインポート
  * 分布モデルの可視化
  * 離散化レベルを手動で選ぶUI
  * 離散化レベルを組み合わせるアルゴリズム
  * 所与の指標に対する最適値を導出するための組合せ的手法と進化的手法
  * OpenSCENARIO形式で具体シナリオをエクスポート

確率論的モジュールを使うことで、論理シナリオをインポートして、テスト用のパラメタセットを発生確率pをもとに導出し、結果的に、具体シナリオをOpenSCENARIO形式でエクスポートする。

### 結果評価
* 2次元ヒートマップを使った結果表現
複数パラメタに対する具体シナリオの危険度。カラーレンジは緑の安全から赤の危険まで。

* 特定のシナリオに対する詳細分析（信号の時系列）
具体シナリオに対する種々の指標の時系列表示。
以下の値をプロット：
 * TTC
 * 相対距離（TimeHeadWay）
 * 自車と障害物の相対速度
 
 # 14. Interdisciplinary Test Infrastructures / Maps
 ## テストケース用の実路および高精度マップ生成。実路とテストテストシナリオからシミュレーション用道路の作成方法
 
 ### 道路生成：
 あらゆる種類の道路を表現するために、自動的にパラメタを生成する方法を用いる。これはドイツにおける標準的な交差点にもとづいたもので、走行コースと交通基盤とも適合している。  
 結果：シミュレーションでのテストカバレッジに必要な全種類の道路を生成することができる。
### 道路のマッピング：
高速道路の大規模な区間を高精度マッピングすることで、実世界のデータを用意する。道路生成と類似したオブジェクトカタログを広範囲に対象とする。
結果: シミュレーションと実路試験用の広範かつ高精度な高速道路データセット

### 地理データサーバ：
サーバを使ってマッピングした道路と生成済みの道路をOpenDRIVE形式で展開する。エリア、ルート、または特定の性質にもとづいて選んだ道路データを計画する。

## パラメトリックな道路生成で全種類の根拠ある道路をつくる
簡単なXMLを使った道路記述で道路断面と道路インフラを定義する。ツールチェーンでOpenDRIVEの道路ネットワークに変換する。

### システマチック
* 道路断面と既定の横断面を自動的かつマクロベースで生成する
* 以下を追加して道路としての精度を上げる：
  * 高度（凹凸）
  * コース（カーブ）
  * 横断プロファイル
  * 車線と区画線
* 道路標識、設備（防音壁、ガードレールなど）、植樹帯といった道路付属物
* OpenDRIVEでつくった道路断面

### 統合
変換プロセスはバッチプロセスとしてとして実装され、かつサーバを使った完全自動サービスとしても実装される。

### 今後の開発
横断面、自動車専用道の出入口のランプウェイ等の曲線生成を強化する予定。

## 自動生成した道路を用いたシミュレーションの妥当性確認に向けて、多くの種類の高速道路を実路データの基礎としてマッピングする
### 道路マッピング
移動式のマルチセンサを使って、車道、構造的特性、また道路インフラをマッピングする。
違った種類の高速道路を含む大規模なテスト領域を表現し、シミュレーションと実路試験で利用する。
### データ処理
* 地上基準点による高精度な軌跡処理
* マッピング走行時の全データの均質化
* 車線とオブジェクトに関して処理した生データの評価（オブジェクトカタログに基づいて）
* OpenDRIVE形式にデータを変換・展開

# 15. Sensor Simulation Models
周辺のオブジェクトと交通参加者はレーダ、カメラ、LiDARセンサのセンシング対象である。これらセンサのコンセプトは違えど、オブジェクトの位置と速度の計測精度、分類に関して補完し合う関係にある。センサモデルによって、交通環境が異なるシナリオに対してシミュレーション環境の中でセンサのふるまいバーチャル試験が可能になる。  
センサモデルを用いて、自動運転機能のバーチャル試験・妥当性確認ができるようになり、自動運転の市場投入には欠かせない。

## 環境シミュレーションからセンサデータをシミュレーションする
レーダ・カメラ・LiDARセンサモデルが開発され、それぞれのセンサの属性が反映されている：
### センサ共通
* 距離
* 検知範囲（FOV）
* 遮蔽
...
レーダ、LiDAR、およびカメラに対応するセンサモデルの要素は、それぞれの物理的設定による。レーダとカメラについては、現象論的にそのふるまいをモデル化する、つまり、ふるまいの結果をモデル化するということになる。  
レーダセンサモデルには、シミュレーションに登場する全車両に関するRCS（レーダ断面）の極座標表示が求められる。  
CarMakerを使ったセンサデータの可視化の一例：前方の青い車両が認識され緑色の枠で囲まれている。枠の右側にあるテキストボックスにはセンサ特有の結果が表示されている。

## オブジェクト固有のレーダ断面（RCS）の求め方
レーダシステムシミュレーションで使う車両のRCSモデルを精度良く求める。

### 目的
* 散乱中心（scattering center）をいくつか設定して、レーダの（複雑な）散乱特性を精度良く表現する
* CADの車両モデルをシミュレーションで使う代わりに、散乱中心モデルを使うことで、計算時間を減らす  
シミュレーションで車両の散乱中心（緑色の点）を特定する。全散乱中心を足し合わせることで車両全体として精度の良いモデルを作ることができる。

### プロセス
* 幾何光学（geometrical optics; GO）法とMoM法（method of moments）によるSBR法（shooting and bouncing ray）
* 実車計測による妥当性確認  
車両に対するレーダの後方散乱（back scattering）特性は、角度依存性のある後方散乱の振幅である（単位 dB m^2）。この図では単一の静的なRCS（送受信の位置が同じ）を示している。マルチパス伝搬による影響のため、バイスタティック（bistatic）RCS（送受信の位置が異なる）が必要で、これは散乱中心モデルから求められる。

### 見通し
CADの車両モデルを用いて散乱中心を抽出する（シャシ底面も含む!）汎用的手法  
シャシ底面に散乱中心を配置することで、地面（道路）反射による影響を考慮することもできる。

## 安全性評価に関するセンサデータの不確実性
環境認識は異なる領域の不確実性の影響を受ける：

### 状態の不確実性
オブジェクト位置、オブジェクト速度といった状態変化における不確実性。現在使われているセンサ技術の確率論的計測誤差によるもの。

### 分類の不確実性
クラス分類の不確実性（たとえば、乗用車とトラックの分類）。オブジェクト検知アルゴリズムまたはセンサの限界による誤った分類で起こる。

### 存在の不確実性
実際のセンサがオブジェクトを捕捉しているか否かという不確実性。個々のセンサの検知の不確実性によって起こる。

センサモデルは実際のセンサと同じ不確実性のふるまいを示さなくてはならない。

# 16. Software-in-the-Loop

## PEGASUS では既存のツールへの接続を可能にする
既存のSILツールをツールチェーンと実路試験に統合する方法

### 背景
* 共通のツールチェーンによるテスト実行
* ツールチェーン内の共通インタフェースでツールを横断する
...
## PEGASUSツールチェーンをHILシミュレーション環境に実現する。

HAD機能をHIL環境でテストする方法

### 論理層
ECU-TESTの確率論的モジュールを使って、論理シナリオを具体シナリオに処理する。OpenSCENARIO、OpenDRIVE、およびROAD5ファイル形式による具体シナリオをTestRunとROAD5フォーマットに変換し、実時間条件下のHILテストベンチで実行する。ECU-TESTの危険度指標を使ってHAF機能のふるまいを評価・表示する。

### 技術層
HILシミュレータの典型的構造として、XPACK4の実時間システム、複数の制御ユニット、およびシミュレーション制御PCと同様のCarPCによる構成がある。実時間システムによって、HAF機能が載っている複数のECUとCarPCをCAN経由で通信する。実時間システムは、シミュレーション制御PCによって遠隔制御され、このPCではCarMakerの画面表示とECI-TESTの実行がなされている。

### HIL環境の本質は何か？
HIL環境と比較して、センサ、ECU、アクチュエータはテスト対象のシステムの一部である。これにより、テスト対象のシステムをハードウェア要素とその影響―たとえば限られた計算能力やストレージ容量による望ましくない性能―へと拡がる。ハードウェア要素間の通信もまたテストの一部分である。メッセージの消失、送信の時間遅れ、または限られた信号値域といった影響は自動的に考慮される。  
HILテストベンチの設計次第では、HAFシステムの全ハードウェア要素―センサからECU、そしてアクチュエータに至るまで―を対象としたほぼ完全なテストが可能である。HILテストと実際の運転テストの違いというものは、このように最小限に減らすことができる。SIL環境と比べて、シミュレーションモデルの依存度は小さくなっている。とくに実センサと統合したセンサモデルがそうだ。

# 18. Proving Ground - Generic Approach of a Test Case for Proving Ground
## テストコース試験に対する一般的アプローチ
具体シナリオの結果に対して必要な要素は何か？  
テスト手順はどのようなものか？

具体シナリオには様々なテスト要素が含まれる：
* A. テスト対象  
PEGASUSではテスト対象車両（VUT、自車両）、しかしたとえば可変路面標示（つまり、道路インフラとして）といったものも考えられる。
* B. 基本道路  
道路に固有のパラメタ、たとえば寸法（コース）と道路トポロジ（車線とその幅員、路面標示等からなる交差点）。
* C. 案内インフラ  
固定・変動交通標示、信号機で道路に恒久的に敷設されているもの
* D. 一時的設置物  
ボトルネックや道路作業のために基本道路が拡張され、適したシナリオを生成する。
* E. 静止物  
構造的要素と道路に恒久的に敷設された物体。たとえば、ガードレール、橋梁など。
* F. 移動物  
移動可能な物体。速度ゼロの車両は静止している移動物で、速度が非ゼロのときには動的な移動物となる。
* G. 環境  
環境条件。

## PEGASUS 管制センター
一般的手順の実装例  
HADのテスト実施に必要な計測機器を準備する。

### 管制センター
* 様々なテストコースでの利用に対応した機動性デザイン
* テスト手順の管制とモニタリング
* テストに関連する車両・装置との双方向のデータ通信
* 管制ステーションソフトウェアを備えた強力なコンピュータ
* ワイヤレス通信ネットワーク
* 自律制御

### TSV Traffic Simualtion Vehicle
* 移動物体
* RTK（real time kinematics）衛星ナビゲーションと内部プラットフォームによる高精度自己位置推定
* 通信ユニット
* 位置ベースの軌道制御
* 内部車両を直接制御
* ブレーキロボットによる減速度の間接制御
...
* テスト対象車両
* RTK 衛星ナビゲーションと内部プラットフォームによる自己位置推定
* 通信ユニット

管制センター、TSV、およびVUT間のワイヤレス通信の遅延が少ないこと。
